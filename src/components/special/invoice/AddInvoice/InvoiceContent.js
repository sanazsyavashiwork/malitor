// InvoiceContent.js - اصلاح شده
"use client";
import React from "react";
import { useToast } from "@/hooks/useToast2";
import { useInvoice } from "./InvoiceContext";
import { usePostInvoicesQuery } from "@/hooks/Invoices/usePostInvoicesQuery";
...
const InvoiceContent = () => {
  ...
  const { mutate } = usePostInvoicesQuery({});

  const handleAddInvoice = async ({ paymentData, completeData, id }) => {
    try {
      // تبدیل تاریخ‌ها به فرمت مطلوب
      const processedData = {
        ...completeData,
        // تبدیل date اصلی
      ...
      mutate(processedData, {
        onSuccess: (data) => {
          console.log("Invoice submission success:", data.data.id);
          successToast("پیش صورت حساب با موفقیت ثبت شد");
          window.location.href = `${ROUTES.PRIVATE.INVOICE_PRINT}/${data.data.id}`;
        },
        onSettled: () => {
  ...
  return (
    <>
      <AddInvoiceInformationPart1 />
      <PaymentInfo handleAddInvoice={handleAddInvoice} />
    </>
  );
};
