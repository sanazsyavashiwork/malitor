...
import { useGetCustomerQuery } from "@/hooks/Customers/useGetCustomerQuery";
import { usePutCustomerQuery } from "@/hooks/Customers/usePutCustomerQuery";
import { useErrorApiHandler } from "@/hooks/useErrorApiHandler";
import Loading from "../../loading/loading";

const EditCustomers = ({ id }) => {
  const { customer: businessPartner, isLoadingCustomer } = useGetCustomerQuery({
    id,
  });
  const { successToast, errorToast } = useToast();
  const { mutate, isPending } = usePutCustomerQuery({ id });
  const { serverErrors, clearErrors, handleError } = useErrorApiHandler();

  const initialValues = {
    // Checkbox fields
    active: businessPartner?.data?.active ?? false,
    customer: businessPartner?.data?.customer ?? false,
    provider: businessPartner?.data?.provider ?? false,
    customerType: businessPartner?.data?.customerType ?? "",

    // Text fields
    name: businessPartner?.data?.name ?? "",
    identificationCode: businessPartner?.data?.identificationCode ?? "",
    phoneNumber: businessPartner?.data?.phoneNumber ?? "",
    faxNumber: businessPartner?.data?.faxNumber ?? "",
    mobileNumber: businessPartner?.data?.mobileNumber ?? "",
    nationalId: businessPartner?.data?.nationalId ?? "",
    economicCode: businessPartner?.data?.economicCode ?? "",
    registrationNumber: businessPartner?.data?.registrationNumber ?? "",
    passportNumber: businessPartner?.data?.passportNumber ?? "",
    email: businessPartner?.data?.email ?? "",
    postalCode: businessPartner?.data?.postalCode ?? "",
    province: businessPartner?.data?.province ?? "",
    city: businessPartner?.data?.city ?? "",
    address: businessPartner?.data?.address ?? "",
  };
  const onSubmitHandler = (values, { setSubmitting, resetForm }) => {
    ...
    mutate(values, {
      onSuccess: (data) => {
        resetForm();
        successToast("مشتری با موفقیت ویرایش شد");
      ...
      onError: (error) => {
        handleError(error);
        errorToast("ویرایش مشتری با خطا روبرو شد");
      },
    });
  };
